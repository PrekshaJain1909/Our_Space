    ...
    <div className="hz-modal-backdrop">
      <div className="hz-modal">
        ...
        <div className="hz-modal-actions">
          <button type="button" className="hz-secondary-btn" onClick={onCancel} disabled={loading}>{cancelLabel}</button>
          <button type="button" className="hz-primary-btn" onClick={onConfirm} disabled={loading}>{loading ? 'Saving…' : confirmLabel}</button>
        </div>
      </div>
    </div>
  );
