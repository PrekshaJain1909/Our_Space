...
export default function BucketPage() {
  ...
  const [weddingItems, setWeddingItems] = useState([]); // { _id, type, title, description, image, referenceLink }
  ...
  const handleDeleteWeddingItem = async (item) => {
    const id = item && (item._id || item.id);
    if (!id) return;
    try {
      await bucketApi.deleteVisionItem(id);
      setWeddingItems((prev) => prev.filter((entry) => (entry._id || entry.id) !== id));
    } catch (e) {
      console.warn('[Bucket] delete wedding vision failed', e);
    }
  };
  ...
  React.useEffect(() => {
    ...
    const loadWeddingItems = async () => {
      try {
        const res = await bucketApi.getVisionItems();
        const items = res.data?.data || res.data;
        if (mounted) setWeddingItems(items || []);
      } catch (e) { console.warn('Could not load wedding vision items', e); }
    };
        ...
        <section className={`bucket-sections ${activeTab === 'both' ? 'two-col' : 'single-col'}`}>
          {activeTab === "bucket" && (
            <div className="bucket-section bucket-list-section">
              ...
              <header className="healing-hero-card">
                ...
                <p className="healing-badge">📝 Bucket List</p>

                <h1 className="healing-title">
                  Goals, adventures and couple challenges.
                </h1>
              </header>
          ...
          {activeTab === "wedding" && (
            <div className="bucket-section wedding-vision-section">
              ...
              <header className="healing-hero-card">
                <div className="healing-hero-glow" />
                <p className="healing-badge">🤵🏻👰🏻 Wedding Vision</p>
                <h1 className="healing-title">
                  Inspiration, photos and vendor links.
                </h1>
